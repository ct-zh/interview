...
innodb底层特性: 插入缓冲、两次写、自适应哈希索引、异步 IO、刷新邻接页、预读;
...
三星索引通俗版:
1. 一星：WHERE 条件能通过索引取出需要的数据
2. 二星：将排序操作（ORDER BY）的列加入到索引中
3. 三星：可以通过索引直接取出整个 SQL 语句需要的数据，不必回表
